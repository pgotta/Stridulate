...
object OpenSetDecision {
    fun evaluate(
        ...
        val requiredConfidence = maxOf(policy.minimumConfidence, safetyRule.minimumConfidence)
        val requiredMargin = maxOf(policy.minimumMargin, safetyRule.minimumMargin)
        ...
        val reason = when {
            ...
            tier == ReliabilityTier.GOOD ->
                "Possible match with useful independent support. Confirm with season, range and the reference call."
            else ->
